...
class Unpacker(abc.ABC):
    ...
    def plot(self, table):
        ...
        for i, (period, period_explanation) in enumerate(explanation.sort_values(self.period).groupby(self.period)):
            if i > 0:
                ...
                contributions['end'] = prev_total_value + contributions['impact'].cumsum()
                contributions['start'] = contributions['end'].shift(1).fillna(prev_total_value)
                label_cols = [*self.period, *self.group, 'kind']
                contributions['label'] = contributions[label_cols].agg(lambda x: ' • '.join(x.astype(str)), axis='columns')
                contributions['is_positive'] = contributions['impact'] > 0

                chart = alt.Chart(contributions).mark_bar().encode(
                    y=alt.Y(
                        'label:O',
                        sort=None,
                        axis=alt.Axis(
            ...
            total = pd.DataFrame({
                'label': [period],
                'total': (
                    [(period_explanation['count'] * period_explanation['ratio']).sum() / period_explanation['count'].sum()]
...
class SumExplainer(Unpacker):
    ...
    @property
    def _necessary_columns(self):
        return [self.fact, *self.period, *self.group, *([self.count] if self.count else [])]
    ...
    def _explanation(self, table):
        explanation = table.aggregate(
            ...
            count=(
                table[self.count].sum()
                if self.count
                else table[self.fact].count()
            )
        )
...
class MeanExplainer(Unpacker):
    ...
    def _explanation(self, table):
        ...
        if (by := [*self.group, *self.period[1:]]):
            explanation = explanation.group_by(by).order_by(self.period).mutate(
        ...
        return explanation.mutate(
            ...
            mix=(explanation['share'] - explanation['share_lag']) * (explanation['ratio_lag'] - explanation['global_ratio'])
        )
...
class FunnelExplainer(Unpacker):
    ...
    def _explanation(self, table):
        ...
        explanation = explanation.group_by([*self.group, *self.period[1:]]).order_by(self.period).mutate(**{
            ...
            f'{ratio_name}_contribution': functools.reduce(
                ...
                [
                    *[explanation[ratio_name] for ratio_name in ratio_names[:i]],
                    explanation[ratio_names[i]] - explanation[f"{ratio_names[i]}_lag"],
                    *[explanation[f'{ratio_name}_lag'] for ratio_name in ratio_names[i+1:]]
                ]
            )
    ...
    def _format(self, explanation):
        return (
            ...
            .select([
                *self.period,
                *self.group,
                *[col for col in explanation.schema() if col.endswith('_contribution')]
            ])
            .dropna(how="any")
        )
